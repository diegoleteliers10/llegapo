...
export default function Home() {
  ...
  // Inicializar búsquedas recientes desde localStorage
  ...
  const removeRecentSearch = (searchToRemove: RecentSearch, e: React.MouseEvent) => {
  ...
  const getWeatherIconComponent = (iconType: WeatherIconType) => {
    const iconProps = {
      className: "w-8 h-8 sm:w-8 sm:h-8 shrink-0 transition-all group-hover:drop-shadow-[0_0_12px_currentColor]",
    };

    switch (iconType) {
      case "sun":
        return <Sun {...iconProps} className={`${iconProps.className} text-yellow-400 group-hover:text-yellow-300`} />;
      case "cloud":
        return <Cloud {...iconProps} className={`${iconProps.className} text-gray-400 group-hover:text-gray-300`} />;
      case "cloud-rain":
        return <CloudRain {...iconProps} className={`${iconProps.className} text-blue-400 group-hover:text-blue-300`} />;
      case "snowflake":
        return <Snowflake {...iconProps} className={`${iconProps.className} text-cyan-400 group-hover:text-cyan-300`} />;
      case "wind":
        return <Wind {...iconProps} className={`${iconProps.className} text-gray-300 group-hover:text-gray-200`} />;
      default:
        return <Sun {...iconProps} className={`${iconProps.className} text-yellow-400 group-hover:text-yellow-300`} />;
    }
  };
  ...
  return (
    <div className="min-h-screen flex flex-col map-bg">
      ...
      {/* Main Content */}
      <main className="flex-grow flex flex-col items-center justify-center px-4 py-12 pb-32 sm:pb-40 relative overflow-visible">
        ...
        <div className="w-full max-w-4xl glass-panel rounded-2xl p-8 md:p-12 relative z-10 mb-6">
          ...
          {/* Tabs Navigation */}
          {/* Comentado temporalmente - no se usará por ahora */}
          {/* <div className="mb-8">
            <div className="flex justify-center gap-1 sm:gap-2 p-0.5 sm:p-1 bg-black/20 backdrop-blur-md rounded-xl w-full sm:w-fit mx-auto border border-white/5">
              ...
              <button
                ...
                className={`px-3 py-2 sm:px-6 sm:py-2.5 rounded-lg text-xs sm:text-sm font-medium transition-all flex-1 sm:flex-none ${
                  activeTab === "recorrido"
                    ? "bg-white/10 text-white shadow-sm border border-white/10"
                    : "text-white/50 hover:text-white hover:bg-white/5"
                }`}
            ...
            <div className="mt-10 pt-8 border-t border-white/10">
              ...
              <div className="flex flex-wrap gap-3">
                ...
                {(showAllRecentSearches 
                  ...
                  const shouldHideOnMobile = !showAllRecentSearches && index === 3;
                    ...
                    >
                      ...
                      <div className="flex flex-col text-left">
                        <span className="text-sm font-bold text-white">
                          {search.stop}
                          {search.busId && (
                            <span className="text-primary ml-1">• {search.busId}</span>
                          )}
                        </span>
                        <span className="text-[10px] text-white/50">
                          {search.busId ? "Paradero + Recorrido" : "Solo Paradero"}
                        </span>
                      </div>
                    </button>
                  );
                ...
                {showAllRecentSearches && recentSearches.length > 3 && (
                  ...
                  >
                    ...
                    <span className="text-sm font-bold text-white">Ver menos</span>
                  </button>
                )}
              </div>
            </div>
        ...
        <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6 opacity-60 relative z-10 mt-5 mb-5">
          {metroStatusQuery.isLoading ? (
            <div className="glass-panel p-5 sm:p-4 rounded-xl flex items-center gap-4 sm:gap-4 border-none bg-black/40 group transition-all hover:opacity-100">
              ...
              <div className="min-w-0">
                <h4 className="text-white font-bold text-sm sm:text-sm">Cargando estado...</h4>
            ...
            <div className="glass-panel p-5 sm:p-4 rounded-xl flex items-center gap-4 sm:gap-4 border-none bg-black/40 group transition-all hover:opacity-100">
              ...
              <div className="min-w-0">
                <h4 className="text-white font-bold text-sm sm:text-sm">Metro Operativo</h4>
                <p className="text-white/50 text-xs sm:text-xs">
            ...
            const hasAlterations = metroLines.some(line => 
              !line.status.toLowerCase().includes("sin alteraciones") && 
              line.status.toLowerCase() !== "sin alteraciones"
            );
            const linesWithAlterations = metroLines.filter(line => 
              !line.status.toLowerCase().includes("sin alteraciones") && 
              line.status.toLowerCase() !== "sin alteraciones"
            );
          ...
          {deviationsQuery.isLoading ? (
            <div className="glass-panel p-5 sm:p-4 rounded-xl flex items-center gap-4 sm:gap-4 border-none bg-black/40 group transition-all hover:opacity-100">
              <AlertTriangle className="w-8 h-8 sm:w-8 sm:h-8 text-yellow-400 animate-pulse shrink-0" />
              <div className="min-w-0">
                <h4 className="text-white font-bold text-sm sm:text-sm">Cargando desvíos...</h4>
                <p className="text-white/50 text-xs sm:text-xs">
            ...
            <div className="glass-panel p-5 sm:p-4 rounded-xl flex items-center gap-4 sm:gap-4 border-none bg-black/40 group transition-all hover:opacity-100">
              <CheckCircle className="w-8 h-8 sm:w-8 sm:h-8 text-green-400 shrink-0" />
              <div className="min-w-0">
                <h4 className="text-white font-bold text-sm sm:text-sm">Sin desvíos</h4>
                <p className="text-white/50 text-xs sm:text-xs">
            ...
            >
              <AlertTriangle className="w-8 h-8 sm:w-8 sm:h-8 text-yellow-400 transition-all group-hover:text-yellow-300 group-hover:drop-shadow-[0_0_12px_rgba(250,204,21,0.8)] shrink-0" />
              <div className="min-w-0">
                <h4 className="text-white font-bold text-sm sm:text-sm">Desvío en progreso</h4>
                <p className="text-white/50 text-xs sm:text-xs">
          ...
          <div className="glass-panel p-5 sm:p-4 rounded-xl flex items-center gap-4 sm:gap-4 border-none bg-black/40 group transition-all hover:opacity-100">
            {weatherQuery.isLoading ? (
              <>
                ...
                <div className="min-w-0">
                  <h4 className="text-white font-bold text-sm sm:text-sm">Cargando clima...</h4>
                  <p className="text-white/50 text-xs sm:text-xs">
              ...
              <>
                ...
                <div className="min-w-0">
                  <h4 className="text-white font-bold text-sm sm:text-sm">
                    {weatherQuery.data.temperature}°C {weatherQuery.data.description}
                  </h4>
                  <p className="text-white/50 text-xs sm:text-xs">
              ...
              <>
                ...
                <div className="min-w-0">
                  <h4 className="text-white font-bold text-sm sm:text-sm">Clima no disponible</h4>
                  <p className="text-white/50 text-xs sm:text-xs">
